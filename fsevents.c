...
//  //  }
//  // } else {
//  //  printlogf(L, "Error", "unknown event(%d) in fsevents.", atype);
//  //  exit(-1); // ERRNO
//  // }
...
//    if (aflags & FSE_COMBINED_EVENTS) {
//      printlogf(L, "Error", "Fsevents: combined events");
//    }

//    if (aflags & FSE_CONTAINS_DROPPED_EVENTS) {
//      printlogf(L, "Error", "Fsevents: contains dropped events");
//    }
...
//      printlogf(L, "Error", "atype (%d), etype (%s)", atype, etype);
//      exit(-1);
//    }
//    if (isdir < 0) {
...
static void
handle_event(file_paths_t *file_paths, char *path)
{
...
{
  printlogf(L, "Warn", "Call fsevents_ready");
  printf("HERE fsevents_ready\n");
...
)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    // close_exec_fd(ids[i]);
    // non_block_fd(ids[i]);
    printf("before observe_fd\n");
    observe_fd(ids[i], fsevents_ready, NULL, fsevents_tidy, NULL);
    printf("after observe_fd\n");
    // printf("length: %l\n", file_paths->len);
...
open_fsevents(lua_State *L)
{
  ...
  //   L,
  //   "Warn",
  //   "initialize FSEventStreamContext"
  // );
  ...
  dir_path = "/Users/vadim/projects/cw-dev-api";
  // stat_value = stat(dir_path, &stat_data);

  printlogf(L, "Warn", "stat: %d", stat_value);

  // if (stat_value < 0) {
  //   // File doesn't exist yet. Watch parent dir instead.
  //   stat_data.st_mode = S_IFREG;
  //   printlogf(L, "Warn", "hit here 1");
  // }

  // if (stat_data.st_mode & S_IFREG) {
  //   printlogf(L, "Warn", "hit here 2");
  //   handle_event(file_paths, dir_path);
  // }
  handle_event(file_paths, dir_path);
  ...
  printlogf(L, "Warn", "create FSEventStreamRef");

  FSEventStreamRef stream;
  FSEventStreamCreateFlags flags = kFSEventStreamCreateFlagFileEvents;

  printlogf(L, "Warn", "paths: %d", (int32_t)sizeof(paths));

  stream = FSEventStreamCreate(NULL, &event_callback, &ctx, paths, kFSEventStreamEventIdSinceNow, 0, flags);
  rl = CFRunLoopGetCurrent();
  FSEventStreamScheduleWithRunLoop(stream, rl, kCFRunLoopDefaultMode);
  FSEventStreamStart(stream);

  printlogf(L, "Warn", "before loop run");
